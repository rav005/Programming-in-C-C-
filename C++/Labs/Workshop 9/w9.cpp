...
w9::List<w9::Product> merge(const w9::List<w9::Description>& desc, const w9::List<w9::Price>& price) {
    ...
    for(int d=0; d < desc.size(); d++){ //Going through Description object.
       int p;
		for(p=0; p < price.size(); p++){ //Going through Price object.
            if(desc[d].code == price[p].code){ //If common element matches makes
                                                //a new object in temp object. 
                std::unique_ptr<w9::Product> temp(new w9::Product(desc[d].desc, price[p].price));
                temp->validate();
